...
@admin_bp.route("/api/run-news-analysis", methods=['POST'])
@login_required
def run_news_analysis():
    """API endpoint to trigger news analysis"""
    try:
        # Import the news analysis function
        from routes.news_analysis import perform_news_analysis
        
        # Run the analysis
        success = perform_news_analysis()
        
        if success:
            return jsonify({"success": True, "message": "Analysis completed successfully"})
        else:
            return jsonify({"success": False, "message": "Analysis failed. Check logs for details."}), 500
    except Exception as e:
